...
class CompressionTest(unittest.TestCase):
    def _test_compress(self, compress_algorithm):
        ...
        compressed_buffer = compress_algorithm(default_buffer, len(default_buffer))
    ...
    def _test_decompress(self, compress_algorithm, decompress_algorithm):
        default_buffer = "AAAAAAAA"*90
        compressed_buffer = compress_algorithm(default_buffer, len(default_buffer))
        decompressed_buffer = decompress_algorithm(compressed_buffer, len(compressed_buffer))
    ...
    def test_efi_decompress(self):
        self._test_decompress(efi_compressor.EfiCompress, efi_compressor.EfiDecompress)

    def test_tiano_compress(self):
        self._test_compress(efi_compressor.TianoCompress)

    def test_tiano_decompress(self):
        self._test_decompress(efi_compressor.TianoCompress, efi_compressor.TianoDecompress)

    def test_lzma_compress(self):
        default_buffer = "AAAAAAAA"*90
        compressed_buffer = efi_compressor.LzmaCompress(default_buffer, len(default_buffer))
    ...
    def test_lzma_decompress(self):
        default_buffer = "AAAAAAAA"*90
        compressed_buffer = efi_compressor.LzmaCompress(default_buffer, len(default_buffer))
        decompressed_buffer = efi_compressor.LzmaDecompress(compressed_buffer, len(compressed_buffer))
